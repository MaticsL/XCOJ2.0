...
if(isset($_GET['search'])&&trim($_GET['search'])!=""){
    ...
    $filter_sql=" ( title like '%$search%' or source like '%$search%' or tags like '%$search%')";
...
while ($row=mysql_fetch_object($result)){
	...
	$view_problemset[$i][0]=$row->problem_id;
	$view_problemset[$i][1]=$row->title;
	$view_problemset[$i][2]=$row->source;	
	$i++;
}
mysql_free_result($result);
$api=json_encode($view_problemset);
echo $api;
?>
